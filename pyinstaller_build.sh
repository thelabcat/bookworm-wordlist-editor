...
output_name="bookworm_wordlist_editor-$os_suffix-$(uname -m)"
...
echo "Building exe"
pyinstaller -F --icon=bookworm_wordlist_editor.ico --add-data bookworm_wordlist_editor.png:. --add-data large_en.msgpack.gz:wordfreq/data/ --name "$output_name" bookworm_wordlist_editor.pyw 2>&1 | tee pyinstaller_build_log.txt
